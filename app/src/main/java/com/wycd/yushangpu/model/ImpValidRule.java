...
public class ImpValidRule {
    public void valiRule(final Activity ac, String VGID, String PGID,String PTGID,
...
//        PGID	商品GID	string	是
        ...
        client.post(url, params, new AsyncHttpResponseHandler() {
            ...
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                try {
                    LogUtils.d("xxValiRuleS", new String(responseBody, "UTF-8"));
                    JSONObject jso = new JSONObject(new String(responseBody, "UTF-8"));
                    if (jso.getBoolean("success")) {
                        Gson gson = new Gson();
                        Type listType = new TypeToken<List<ValiRuleMsg>>() {
                        }.getType();
                        List<ValiRuleMsg> sllist = gson.fromJson(jso.getString("data"), listType);
...
//                    com.blankj.utilcode.util.ToastUtils.showShort("获取有效提成部门列表失败");
                    back.onErrorResponse("");
                }
            }
