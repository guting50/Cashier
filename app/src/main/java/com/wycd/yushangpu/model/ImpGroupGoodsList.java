...
public class ImpGroupGoodsList {

    public void getGroupGoodsList(final Activity ac, String groupGid ,final InterfaceBack back) {
        // TODO 自动生成的方法存根
        ...
        client.post(url, params, new AsyncHttpResponseHandler() {
            ...
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                try {
                    LogUtils.d("xxgroupgoods", new String(responseBody, "UTF-8"));
                    JSONObject jso = new JSONObject(new String(responseBody, "UTF-8"));
                    if (jso.getBoolean("success")) {
                        Gson gson = new Gson();
                        Type listType = new TypeToken<List<ShopMsg>>() {
                        }.getType();
                        List<ShopMsg> sllist = gson.fromJson(jso.getString("data"), listType);
...
//                    ToastUtils.showToast(ac,"获取多规格商品失败");
//                    com.blankj.utilcode.util.ToastUtils.showShort("获取多规格商品失败");
                    back.onErrorResponse("");
                }
            }
