...
public class ImpEmplList {
    public void emplList(final Activity ac,
        ...
        client.post(HttpAPI.API().GET_EMPLLIST, params, new AsyncHttpResponseHandler() {
            @Override
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                try {
                    LogUtils.d("xxEmplS", new String(responseBody, "UTF-8"));
                    JSONObject jso = new JSONObject(new String(responseBody, "UTF-8"));
                    if (jso.getBoolean("success")) {
                        Gson gson = new Gson();
                        Type listType = new TypeToken<List<EmplMsg>>() {
                        }.getType();
                        List<EmplMsg> sllist = gson.fromJson(jso.getString("data"), listType);
...
//                    ToastUtils.showToast(ac,"获取员工列表失败");
//                    com.blankj.utilcode.util.ToastUtils.showShort("获取员工列表失败");
                    back.onErrorResponse("");
                }
            }
